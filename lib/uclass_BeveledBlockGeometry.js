...
THREE.BeveledBlockGeometry = function ( width, height, depth, bevel, widthSegments, heightSegments, depthSegments ) {
	...
	let width_half = this.width / 2;
	let height_half = this.height / 2;
	let depth_half = this.depth / 2;
	
	let adjWidth = this.width - this.bevel*2;
	let adjHeight = this.height - this.bevel*2;
	let adjDepth = this.depth - this.bevel*2;
	...
	if ( this.bevel > 0 )
	{
		...
		let adjHalfWidth = adjWidth / 2;
		let adjHalfHeight = adjHeight / 2;
		let adjHalfDepth = adjDepth / 2;
	...
	function buildPlane( u, v, udir, vdir, width, height, depth, materialIndex ) {
		...
		for ( iy = 0; iy < gridY; iy++ ) {
			...
			for ( ix = 0; ix < gridX; ix++ ) {
				...
				let face = new THREE.Face4( a + offset, b + offset, c + offset, d + offset );
	...
	function buildBevelSide( longSide, xlo, ylo, zlo, xhi, yhi, zhi, xn1, yn1, zn1, xn2, yn2, zn2 ) {
		...
		let vector;
		let offset = scope.vertices.length;
		...
		let face = new THREE.Face4( offset, offset+1, offset+2, offset+3 );
		let vnorm1 = new THREE.Vector3( xn1, yn1, zn1 );
		let vnorm2 = new THREE.Vector3( xn2, yn2, zn2 );
		...
		for ( let i = 0; i < 2; i++ ) {
			...
			for ( let j = 0; j < 2; j++ ) {
				...
				for ( let k = 0; k < 2; k++ ) {
					...
					scope.vertices.push( vector );
					let vnorm1 = new THREE.Vector3(xsign,0,0);
					
					vector = new THREE.Vector3(xsign*xah,ysign*yh,zsign*zah);
					scope.vertices.push( vector );
					let vnorm2 = new THREE.Vector3(0,ysign,0);
					
					vector = new THREE.Vector3(xsign*xah,ysign*yah,zsign*zh);
					scope.vertices.push( vector );
					let vnorm3 = new THREE.Vector3(0,0,zsign);
